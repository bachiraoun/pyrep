...
#
    ...
    def remove_directory(self, relativePath, clean=False, raiseError=True):
        ...
        LR.release_lock()
        #assert error is None or not raiseError, error
        assert error is None or not raiseError, "Unable to remove directory '%s' (%s)"%(relativePath, error,)
        return error is None, error
    ...
    def rename_directory(self, relativePath, newName, raiseError=True):
        ...
        # return
        #assert error is None or not raiseError, error
        assert error is None or not raiseError, "Unable to rename directory '%s' to '%s' (%s)"%(relativePath, newName, error,)
        return error is None, error
    ...
    def copy_directory(self, relativePath, newRelativePath, overwrite=False, raiseError=True):
        ...
        # return
        #assert error is None or not raiseError, error
        assert error is None or not raiseError, "Unable to copy directory '%s' to '%s' (%s)"%(relativePath, newRelativePath, error,)
        return error is None, error
    ...
    def dump_file(self, value, relativePath,
        ...
        assert not raiseError or error is None, "Unable to dump file '%s' (%s)"%(relativePath, error,)
    ...
    def copy_file(self, relativePath, newRelativePath, force=False, raiseError=True):
        ...
        assert copied or not raiseError, "Unable to dump file '%s' to '%s' (%s)"%(relativePath, newRelativePath, '\n'.join(message),)
        return copied, '\n'.join(message)
    ...
    def rename_file(self, relativePath, newRelativePath, force=False, raiseError=True):
        ...
        # return
        assert renamed or not raiseError, "Unable to rename file '%s' to '%s' (%s)"%(relativePath, newRelativePath, '\n'.join(message),)
    ...
    def remove_file(self, relativePath, removeFromSystem=False, raiseError=True):
        ...
        if os.path.isfile(os.path.join(fPath,self.__fileLock%fName)):
            os.remove(os.path.join(fPath,self.__fileLock%fName))
        # return
        assert removed or not raiseError, "Unable to remove file '%s' (%s)"%(relativePath, '\n'.join(message),)
